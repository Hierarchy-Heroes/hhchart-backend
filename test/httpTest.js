...
const { isAncestor } = require('../verification'); 
    ...
    it('authenticated user can access tree data', (done) => {
        request(app)
            ...
            .expect((res) => {
                ...
                if ((res.body.length <= 0 || res.body.length > 2)) throw new Error('Data is incomplete or incorrectly formated');
...
describe('Invalid user is rejected', () => {
    it('authentication does not assign JWT', (done) => {
        request(app)
            .post('/auth/login')
